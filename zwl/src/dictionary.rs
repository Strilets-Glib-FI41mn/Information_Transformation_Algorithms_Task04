...
impl<T: Copy> Dictionary<T>{
    pub fn push(&mut self, word: &(u8, T)){
        self.words.push((word.0, Some(word.1)));
    }
}
impl<T: From<u8> + PartialOrd + Copy + Sub<T, Output = T> + TryInto<usize, Error: std::fmt::Debug> + TryFrom<usize, Error: std::fmt::Debug> + std::fmt::Debug> Dictionary<T>{ //+ Add<T, Output = T>
